...
//JFrame을 상속 받았으므로 자식 클래스인 OrderBasketTest클래스의 인스턴스 변수로 
//부모가 가진 변수나 메소드를 누릴 수 있다.
public class OrderBasketTest extends JFrame {
	String cols[]= {"날짜","판매수량","판매금액"};
	String data[][] = new String [0][3];
	DefaultTableModel dtm_order = new DefaultTableModel(data,cols);
	//서로 연관(의존성)이 있는 클래스 사이에서 별도의 메소드나 코딩 없이 생성자의 파라미터를 통해서 값을 초기화 할 수 있다.		 
	...
	public void dataSetMapping() {
		...
		ArrayList<OrderVO> al = ds.getList2(); //DataSet에서 getList메소드를 호출해서 ArrayList에 담는다
