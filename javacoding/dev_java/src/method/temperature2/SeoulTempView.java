...
public class SeoulTempView implements ActionListener, FocusListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(obj == jtf_date) {
            ta_date = jtf_date.getText(); //JTable = JTextField  
            List<Map<String,Object>> list = null;
            SeoulTempVO stVO = new SeoulTempVO();
            stVO.setSdate(ta_date); //객체로 JTable에 있는 날짜로 setter를 사용해서 sdate를 바꾼다 
        ...
        else if(obj == jcb_year) { //JCombo year를 선택 했을때
            System.out.println(""+jcb_year.getSelectedItem()); //선택된 연정보를 출력한다
            uYear = (String)jcb_year.getSelectedItem(); //파라미터로 전달할 String 변수에 선택된 연정보를 담는다.
            months = stDao.getMonth(uYear); //getter를 이용해 선택한 연도에 해당하는 월 정보를 months 변수에 담는다. 
